...
export default {
  ...
  async execute(member) {
    ...
    const guild = guildsData.find(
      guildData => guildData.id === member.guild.id
    );
    if (guild.noticeTimeoutedMemberRemoved !== null) {
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(
          `サーバー参加日: ${functions.dateToString(member.joinedAt)}\nアカウント作成日: ${functions.dateToString(member.user.createdAt)}\nプロフィール: <@${member.id}>`
        )
        .setColor(
          member.roles.color?.color ? member.roles.color.color : data.greenColor
        );
