      ...
      if (!oldMember.isCommunicationDisabled() && newMember.isCommunicationDisabled()) {
        if (guild.role) {
          try {
            ...
            await newMember.roles.add(role).catch(_error => embed.setDescription(embed.data.description + `\n<@&${role.id}>は権限不足により付与できませんでした。`))
          } catch (error) {
            guildsData.find(guildData => guildData.id === newMember.guild.id).role = null
            functions.writeFile('./data/guilds.json', guildsData)
        ...
        if (guild.role) {
          try {
            const role = await newMember.guild.roles.fetch(guild.role)
            await newMember.roles.remove(role).catch(_error => embed.setDescription(embed.data.description + `\n<@&${role.id}>は権限不足により剥奪できませんでした。`))
          } catch (error) {
            guildsData.find(guildData => guildData.id === newMember.guild.id).role = null
            functions.writeFile('./data/guilds.json', guildsData)
            embed.setDescription(embed.data.description + '\nロールが見つからなかったため、設定はFalseに変更されました。')
